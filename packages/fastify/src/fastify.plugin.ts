import { Plugin, Server, Service } from "dreamsaas";
import {} from '@actual/core/lib/plugins/runnable/runnable.plugin'
        ...
        routes.forEach(route=>{
            ...
            fastify.route({
              ...
              handler: async (request, reply) => {
                ...
                const preReqest = await server.hooks.runHook(beforeHook, {request, data:{}})
                const handler = await server.hooks.runHook(handlerHook, {request, data:preReqest})
                const {data} = await server.hooks.runHook(responseHook, {request, data:handler, reply})
                return data
              }
            })
        })
