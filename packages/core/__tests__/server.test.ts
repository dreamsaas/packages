...
describe('Core Server', () => {
	...
	it('should pass options to external plugin', async () => {
		const server = await createServer({
			plugins: [
				{
					path: path.join(__dirname, './assets/external.plugin.ts'),
	...
	it('should pass options to local plugin', async () => {
		const server = await createServer({
			plugins: [{ id: 'myplugin', options: { content: 'content' } }]
		})
		server.use(
			new (class MyPlugin {
				id = 'myplugin'
	...
	it('should pass config imported hook action options to actions', async () => {
		const server = await createServer({
			hooks: {
				myHook: {
					actions: [
						{ actionId: 'the action', options: { opt: 'custom option' } }
					]
				}
			}
		})

		server.hooks.addHookAction(
			'myHook',
			new Action({
				id: 'the action',
