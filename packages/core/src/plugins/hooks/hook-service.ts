import { Server, Service, Action as iAction } from "dreamsaas";
...
export class HookService implements Service {
  ...
  addHook(hook: HookOptions) {
    ...
    if (originalHook){
      console.warn(`Attempting to add hook with an existing id: ${hook.id}. Using original instead.
  ...
  addHookAction(hookId, action: iAction, actionOptions:{}={}) {
    ...
    const existingAction = this.actions.find(iterAction=>iterAction.id === action.id)
  ...
  async runHook(hookId: string, value?: any) {
    ...
    for (const action of actions) {
      if (typeof action.func === 'function')
        finalValue = await action.func(finalValue,this.server, this.getHookActionOptions(hookId, action.id))
    }
  ...
  getHookActionOptions(hookId:string, actionId:string){
    const hook =  this.getHook(hookId)
    const hookAction =  hook.actions.find(hookAction=>hookAction.actionId === actionId)
