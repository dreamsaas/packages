import { Plugin, PluginConfig, Server } from '@dreamsaas/types'
...
export class ExternalPluginLoaderPlugin implements Plugin {
	...
	async created(server: Server) {
		...
		if (Array.isArray(server.config.plugins)) {
			...
			const organizedPlugins = this.reorganizeAndCheckPluginDependencies(
				importedPlugins
			)
	...
	private async addPlugins(plugins: Plugin[], server: Server) {
		for (let plugin of plugins) {
			const pluginConfig = server.config.plugins.find(
				iPlugin => iPlugin.id === plugin.id
			)
