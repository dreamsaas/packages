...
import { Config, Server } from 'dreamsaas';
...
export const createServer = async (config:Config):Promise<Server>=>{
  ...
  server.log({type:'debug', message:'Plugins loaded after create', context: server.plugins.map(plugin=>plugin.id)} )
  server.log({type:'debug', message:'Registered hooks', context: server.hooks.getHooks() })
